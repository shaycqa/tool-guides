...
- **Headless Templates**: Nuclei can run headless browser templates (`type: headless`) to find vulnerabilities like DOM XSS that require JavaScript execution.
- **Fuzzing Engine**: Nuclei v3 has a built-in fuzzing engine. Look into `-tags fuzz` to fuzz parameters for XSS, SQLi, and SSRF directly without needing a separate tool.
- **Resume Capability**: If your nuclei scan stops halfway, use the `-resume resume.cfg` file generated by nuclei to pick up where you left off.
